...
def main(cfg):
    ...
    if isinstance(cfg.GPU, int):
        gpu_list = str(cfg.GPU)
    else:
        multi_gpus = True
        for i, gpu_id in enumerate(cfg.GPU):
            gpu_list += str(gpu_id)
            if i != len(cfg.GPU) - 1:
                gpu_list += ","
    ...
    save_dir = os.path.join(
        cfg.SAVE_DIR, cfg.MODEL_PRE + "v2_" + datetime.now().strftime("%Y%m%d_%H%M%S")
    )
    ...
    trainset = CASIA_Face(root=cfg.CASIA_DATA_DIR)
    ...
    # nl: left_image_path
    # nr: right_image_path
    nl, nr, folds, flags = parseList(root=cfg.LFW_DATA_DIR)
    testdataset = LFW(nl, nr)
    testloader = torch.utils.data.DataLoader(
        testdataset, batch_size=32, shuffle=False, num_workers=8, drop_last=False
    )
    ...
    exp_lr_scheduler = lr_scheduler.MultiStepLR(
        optimizer_ft, milestones=[36, 52, 58], gamma=0.1
    )
    ...
    for epoch in range(start_epoch, cfg.TOTAL_EPOCH + 1):
        ...
        # train model
        _print("Train Epoch: {}/{} ...".format(epoch, cfg.TOTAL_EPOCH))
        net.train()
        ...
        loss_msg = "    total_loss: {:.4f} time: {:.0f}m {:.0f}s".format(
